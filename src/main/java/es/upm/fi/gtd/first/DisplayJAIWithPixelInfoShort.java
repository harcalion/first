...
/**
 * Clase hija de DisplayJAIWithPixelInfo que implementa un array de short como
 * almacenamiento de los datos de imagen
 * @author Alvar
 */
public class DisplayJAIWithPixelInfoShort extends DisplayJAIWithPixelInfo implements MouseListener,ActionListener
  {
    /**
  ...
  */
  public DisplayJAIWithPixelInfoShort(GeoImg image, JTextField field,
    ...
    if (isDoubleType) 
    	...
    	Raster ras = image.getImage().getData();
    	ipixel_o = new short[width*height*ipixel.length];
		ras.getDataElements(0, 0, width, height, ipixel_o);
    ...
    if (isDoubleType) // process the pixel as an array of double values
      {
    	...
    	String valpixel = new String(ic.buscar(133)); // Tag 133
      	for(int b=0;b<ipixel.length;b++)
      	  {
      		  valpixel = valpixel.concat(ic.buscar(134)
                          +b
                          +": "
                          +ipixel[b]
                          +", "); // Tag 134
 ...
 */
  ...
  private void guardar() {
		...
		principal.getFglobal().setDialogTitle(ic.buscar(99)); // Tag 99
		...
		{		
			...
			{// Guardar como TIFF
				...
				ParameterBlock pb = new ParameterBlock();
		 		switch (profundidad)
		 		{
		 		case ElegirProfundidad.TREINTA:
		 		{		
		 			TiledImage tiled = ElegirProfundidad.crearProfundidadReducida(ipixel_o, profundidad, this.source.getWidth(), this.source.getHeight());
		 			pb.addSource(tiled);			 						 			
		 			break;
		 		}
		 		case ElegirProfundidad.DIECI:
		 		{
		 			
		 			pb.addSource(imagen);
		 			break;
		 		}
		 		case ElegirProfundidad.OCHO:
		 		{
      ...
      /**
     * @deprecated Devuelve los valores de pixel de la imagen contenida en este objeto
  ...
  @Deprecated
   ...
   /**
     ...
     * @return Un short[] con los valores de pixel
     */
