...
*/
public class FractalBoxCounterMod implements PlugInFilter {

	static String sizes = "2,3,4,6,8,12,16,32,64";
	static boolean blackBackground;

	public int[] boxSizes;
	public float[] boxCountSums;
        public float[] logSizes;
	int maxBoxSize;
	int[] counts;
	Rectangle roi;
	int foreground;
	ImagePlus imp;
	int boxSum;
        public ImageProcessor ipo;
        
        public FractalBoxCounterMod (ImagePlus imp, boolean blackBackground,String s)
        {
            if (s.equals(""))
			return;
            boxSizes = s2ints(s);
		...
		if (stats.histogram[0]+stats.histogram[255]!=stats.pixelCount) {
			Dialogos.informarError("8-bit binary image (0 and 255) required.");
			return;
		}
		if (blackBackground)
			foreground = 255;
	...
	public void run(ImageProcessor ip) {
		...
		if (ip.isInvertedLut())
			foreground = 255 - foreground;
		doBoxCounts(ip);
		IJ.register(FractalBoxCounterMod.class);  // keeps this class from being GC'd on 1.1 JVMs
		return;
	}
	...
	void plot() {
        ...
        public String doBoxCounts(ImageProcessor ip) {
		if (!FindMargins(ip))
			return null;
		...
		for (int i=0; i<boxSizes.length; i++) {
			...
			boxCountSums[i] = (float)Math.log(boxSum);
                ...
                int n = boxSizes.length;
                logSizes = new float[n];
                
                for (int i=0; i<n; i++)
			logSizes[i] = (float)Math.log(boxSizes[i]);
                CurveFitter cf = new CurveFitter(Tools.toDouble(logSizes), Tools.toDouble(boxCountSums));
		cf.doFit(CurveFitter.STRAIGHT_LINE);
		double[] p = cf.getParams();
                return IJ.d2s(-p[1],4);
