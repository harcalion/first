 ...
 *
 */
public class Fusion_por_PCAMod implements PlugInFilter {
    ...
    public Fusion_por_PCAMod(int nbits,boolean rescale, boolean interpolar, ImageProcessor [] multi_original, ImageProcessor pan)
    {
        this.nbits = nbits;
        ...
        multibanda = new ImageProcessor[3];
        for (int z=0; z<3; z++) {
            multibanda[z] = multi_original[z].convertToFloat();
    ...
    public ImagePlus []  postPCA(ImageProcessor [] fusionada)
    {
        
        ImagePlus [] resultado = new ImagePlus[fusionada.length];
        for (int z=0;z<fusionada.length;z++) {
            fusionada[z].setMinAndMax(0, java.lang.Math.pow(2, nbits)-1);
            resultado[z] = new ImagePlus("Fusionada PCA",fusionada[z]);
